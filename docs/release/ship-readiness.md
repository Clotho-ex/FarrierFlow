...
| Internal TestFlight group and corrected-build availability | PASS | On 2026-09-07, live App Store Connect showed internal group `FarrierFlow 1.0 RC` with automatic future-build distribution disabled and only build 1.0 (2) in `Ready to Test` state. The group had zero testers; no invitation or physical-device acceptance is inferred. |
...
| TestFlight install and complete physical-iPhone workflow | NOT YET VERIFIED | The internal group and exact build 2 are ready, but the group still has zero testers and no install evidence. Invite the intended internal tester, install build 2 on physical iPhone `00008120-0016043C0138C01E`, record tester/date/device/OS, and exercise onboarding through next appointment, PDF/share, photos, background/foreground, relaunch, Light/Dark, keyboard, touch targets, and outdoor legibility. |
